# 为 Claude Code Agent Teams 编写插件：从 Chorus 的实践谈起
...
2. **以 Chorus 为案例**，展示一个真实的、非 trivial 的插件是怎么利用这套机制解决实际问题的
3. **深入探讨 Sub-Agent 的上下文注入**——这是目前 Claude Code 插件开发中最有技巧性的部分
...
[Chorus](https://github.com/Chorus-AIDLC/chorus) 是一个 AI Agent 与人类的协作平台，实现了 **AI-DLC（AI-Driven Development Lifecycle）** 工作流：
...
位于 `.claude-plugin/plugin.json`，是插件的身份证：
...
插件通过 Marketplace 分发。Marketplace 本质上是一个 JSON 清单文件，可以托管在 GitHub repo、本地目录、或任意 URL：
...
# 3. 选择作用域
/plugin install chorus --scope project  # 项目级（团队共享，提交到 git）
/plugin install chorus --scope local    # 本地级（仅自己）
```

`source` 支持多种来源：本地路径、GitHub repo（`github`）、Git URL（`url`）、npm/pip 包等。
...
**Chorus 的 Skill 体系**：Chorus 包含 7 个参考文档（`references/00` 到 `references/06`），覆盖从公共工具、PM 工作流、Developer 工作流、Admin 工作流，到 Session 管理和 Agent Teams 集成的完整指南。当 Agent 调用 `/chorus` 或 Claude 判断需要 Chorus 知识时，Skill 文档自动加载到上下文中。这相当于给每个 Agent 一本随身携带的操作手册——无论是 Team Lead 还是 Sub-Agent，都能通过 Skill 了解正确的工作流程。
...
| `prompt` | 用 LLM 评估决策 | 需要智能判断时（如代码审查） |
| `agent` | spawn 一个有工具访问权限的 subagent | 需要执行复杂验证时 |

#### Hook 事件全表与 Chorus 用法

下面列出主要的 Hook 事件。对于每个事件，我会说明 Chorus 插件用它来做什么——这些都是真实的、已上线的用法，不是假设。
...
**`SessionStart` — Chorus 用法：Checkin + 上下文注入**

这是插件的"开机自检"。Chorus 在这里做了三件事：

1. 调用 `chorus_checkin()` MCP 工具，获取当前 Agent 的身份（角色、名称、人格）、已分配的 Idea 和 Task、未读通知
2. 将完整的 checkin 结果注入到 Claude 的 `additionalContext` 中——Agent 一启动就知道自己是谁、该做什么
3. 扫描 `.chorus/sessions/` 目录，列出已有的 Sub-Agent session 文件（用于 resume 场景）
...
**`UserPromptSubmit` — Chorus 用法：轻量级状态提醒**
...
**`PreToolUse` — Chorus 用法：工作流引导（3 个子 Hook）**

Chorus 注册了 3 个 `PreToolUse` Hook，分别匹配不同的工具：

| matcher | 脚本 | Chorus 做什么 |
|---------|------|--------------|
| `EnterPlanMode` | `on-pre-enter-plan.sh` | 注入 Chorus Proposal 工作流指南——"先创建 Proposal，设置 Task 依赖 DAG，提交审批后再编码" |
| `ExitPlanMode` | `on-pre-exit-plan.sh` | 提醒检查——"退出 Plan Mode 前确认 Proposal 已创建并提交" |
| `Task` | `on-pre-spawn-agent.sh` | 捕获 Sub-Agent 的 name/type 写入 pending 文件；提醒 Team Lead 在 prompt 中包含 task UUID |

`EnterPlanMode` 和 `ExitPlanMode` 展示了一个有趣的用法：**用 Hook 引导 Agent 遵循特定的工作流程**。当 Agent 进入 Plan Mode 时，Chorus 自动注入"先创建 Proposal 再编码"的指导；退出 Plan Mode 时，检查是否已有 Proposal。这不是强制阻断，而是软性引导。

**`SubagentStart` — Chorus 用法：Session 自动创建 + PE 注入**（核心）

这是 Chorus 插件最核心的 Hook，详见第五章。简要概括：认领 pending 文件 → 创建/复用 Session → 写 session file（含 workflow PE）→ 存储状态映射。

**`SubagentStop` — Chorus 用法：自动清理 + 任务发现**

异步执行，做四件事：(1) 批量 checkout 所有未关闭的 task checkin，(2) 关闭 Session，(3) 清理本地文件和状态，(4) 查询项目中新解除阻塞的 Task 并通知 Team Lead——这一步非常有价值，它实现了**自动任务调度发现**：当一个前置 Task 完成后，下游 Task 自动解除阻塞，Team Lead 立即得到通知可以分配新工作。

**`TeammateIdle` — Chorus 用法：自动心跳**
...
**`SessionEnd` — Chorus 用法：清理 .chorus/ 目录**

会话结束时，检查是否所有 session 文件都已清理、state.json 是否为空。如果是，删除整个 `.chorus/` 目录，不留垃圾文件。

#### Hook 输出格式

Hook 通过 stdout 输出 JSON 来影响 Claude 的行为：
...
- **`systemMessage`**：显示在 Claude Code UI 中的通知
- **`additionalContext`**：注入到 LLM 的 system context 中——这是 Hook 影响 Claude 行为的主要方式
- **`permissionDecision`**：`allow` / `deny` / `ask`，用于控制工具执行权限
- **`suppressOutput`**：设为 `true` 可静默输出（适合高频 Hook 如 heartbeat）

#### 同步 vs 异步

- **同步 Hook**（默认）：阻塞 Claude 直到完成。适合需要立即影响的场景（如 `SubagentStart` 需要在 Sub-Agent 开始工作前创建好 session）
- **异步 Hook**（`"async": true`）：后台运行，不阻塞。适合不影响流程的场景（如 `SubagentStop` 清理资源、`TeammateIdle` 发送心跳）
...
**v0.1.2（最初方案）**——Session file 只有数据：

```json
{
  "sessionUuid": "699f8ed4-...",
  "agentName": "frontend-worker",
  "chorusUrl": "http://localhost:3000"
}
```

Sub-Agent 能拿到 sessionUuid，但不知道该怎么用——需要 Team Lead 在 prompt 里写完整的工作流指令。

**v0.1.3（改进方案）**——Session file 注入 workflow PE：
...
```json
{
  ...
  "workflow": [
    ...
    "",
    "Your Chorus session UUID is: 699f8ed4-...",
    "",
    "--- BEFORE starting work on a task ---",
    "",
    "1. Check in your session to the task:",
    "   chorus_session_checkin_task({ sessionUuid: \"699f8ed4-...\", taskUuid: \"<TASK_UUID>\" })",
    "",
    "2. Move the task to in_progress:",
    "   chorus_update_task({ taskUuid: \"<TASK_UUID>\", status: \"in_progress\", sessionUuid: \"699f8ed4-...\" })",
    "",
    "--- WHILE working ---",
    "",
    "3. Report progress periodically:",
    "   chorus_report_work({ taskUuid: \"<TASK_UUID>\", report: \"...\", sessionUuid: \"699f8ed4-...\" })",
    "",
    "--- AFTER completing the task ---",
    "",
    "4. Check out your session from the task:",
    "   chorus_session_checkout_task({ sessionUuid: \"699f8ed4-...\", taskUuid: \"<TASK_UUID>\" })",
    "",
    "5. Submit for human verification:",
    "   chorus_submit_for_verify({ taskUuid: \"<TASK_UUID>\", summary: \"...\" })",
    "",
...
1. **`workflow` 字段包含完整的工作流指令**——Sub-Agent 读文件后就知道该怎么做
2. **sessionUuid 预填到每个 MCP 调用示例中**——不是占位符，是真实的值
3. **Sub-Agent 只需替换 `<TASK_UUID>`**——这个值由 Team Lead 在 spawn prompt 中提供

这样，Team Lead 的 spawn prompt 从 15 行 boilerplate 缩减到：
...
`on-subagent-start.sh` 中写入 session file 的核心片段：
...
当 Sub-Agent 退出时，`on-subagent-stop.sh`（异步执行）负责清理：
